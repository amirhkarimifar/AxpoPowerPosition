...
- `PowerPosition.Worker` — the deliverable: schedules and runs the extract.
- `PowerPosition.Dashboard` — optional live view of status, logs, and output files.
- `PowerPosition.Tests` — 22 tests, including the two 2026 DST transition dates run against
  the real `PowerService.dll` and a live resilience/retry scenario — the two things this
  exercise is actually testing for.
